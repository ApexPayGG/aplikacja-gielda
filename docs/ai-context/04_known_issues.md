...
| F-004 | Contract alias/type drift | Normalizer layers 2Lâ€“2O |
...
| R-004 | **Single-flight waiter timeout** | Waiters get deterministic fallback, not second Anthropic call â€” do not set `allowFallbackExecution: true`. |
| R-005 | **Redis unavailable** | Single-flight and rate limits fall back to in-memory behavior per process â€” behavior under multi-instance: **UNKNOWN - verify** |
