  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8 px-4 py-10 text-textPrimary">
      <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-brandDark">Koncentracja portfela</h1>
      ...
      {!loading && !error && data && data.stockWeights.length === 0 && (
        <div className="rounded-2xl border border-border bg-bgPrimary p-8 text-center text-textSecondary">Brak danych o pozycjach w portfelu.</div>
      )}

      {!loading && !error && data && data.stockWeights.length > 0 && (
        <>
          <section className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
            <article className="rounded-2xl border border-border bg-bgPrimary p-6 shadow-[0_14px_32px_rgba(45,10,107,0.08)]">
              <h2 className="mb-4 text-base font-semibold text-brandDark">Rozklad sektorowy (placeholder)</h2>
                ...
                <div className="space-y-2 text-sm text-textSecondary">
                  {(data.sectorWeights ?? []).slice(0, 6).map((sector, index) => (
                    <div key={sector.sector} className="flex items-center gap-2">
                      ...
                      <span className="font-semibold text-brandDark">{sector.weight.toFixed(1)}%</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <article className="rounded-2xl border border-border bg-bgPrimary p-6 shadow-[0_14px_32px_rgba(45,10,107,0.08)]">
              <h2 className="mb-4 text-base font-semibold text-brandDark">Diversification score</h2>
              <div
              ...
              <div className="mt-5 text-sm text-textSecondary">
                <div>
                  Wartosc portfela: <span className="font-mono font-semibold text-brandDark">{formatMoney(data.totalValue, currency)}</span>
                </div>
                <div className="mt-1">
                  Liczba pozycji: <span className="font-mono font-semibold text-brandDark">{data.positionCount}</span>
                </div>
              </div>
            </article>
          </section>
          ...
          <section className="rounded-2xl border border-border bg-bgPrimary p-6 shadow-[0_14px_32px_rgba(45,10,107,0.08)]">
            <h2 className="mb-4 text-base font-semibold text-brandDark">Udzial spolek w portfelu</h2>
            <div className="space-y-4">
              {data.stockWeights.map((row) => (
                <article key={row.ticker} className="rounded-xl border border-border bg-bgPrimary p-4">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <div>
                      <div className="font-semibold text-brandDark">{row.ticker}</div>
                      <div className="text-xs text-textMuted">{row.sector}</div>
                    </div>
                    <div className="text-right">
                      <div className="font-mono text-sm font-semibold text-brandDark">{row.weight.toFixed(2)}%</div>
                      <div className="text-xs text-textMuted">{formatMoney(row.value, currency)}</div>
                    </div>
                  </div>
