    ...
    <section className={GLASS_SECTION}>
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        ...
        <span className="inline-flex items-center gap-1 rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[11px] text-white/60">
          <ArrowTrendingUpIcon className="h-3.5 w-3.5 text-[#00C9D4]" aria-hidden />
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <label className="block text-xs font-semibold uppercase tracking-wide text-white/60">
          ...
          <select
            ...
            className="mt-1 w-full rounded-xl border border-white/10 bg-[#2D0A6B]/20 px-3 py-2.5 text-sm text-white backdrop-blur-md focus:border-[#00C9D4]/40 focus:outline-none"
          >
            {PAPER_SYMBOL_PRESETS.map((preset) => (
              <option key={preset} value={preset} className="bg-[#1a0538]">
                {preset}
              </option>
            ))}
          </select>
        ...
        <div className="flex items-end gap-2">
          <button
            ...
            className="w-full rounded-xl border border-white/10 bg-[#2D0A6B]/30 px-3 py-2.5 text-sm font-medium text-white/80 transition hover:border-[#00C9D4]/30"
          >
            Cena mock
          </button>
        </div>
      ...
      <button
        ...
        onClick={handleBuy}
        className="mt-4 inline-flex w-full items-center justify-center rounded-xl border border-[#00C9D4]/35 bg-gradient-to-r from-[#2D0A6B] to-[#00C9D4]/25 px-4 py-3 text-sm font-semibold text-white transition hover:border-[#00C9D4]/55 disabled:cursor-not-allowed disabled:opacity-45 sm:w-auto"
      >
        Kup (Paper BUY)
      </button>
      ...
      {feedback ? <p className="mt-3 text-xs text-[#00C9D4]">{feedback}</p> : null}

      {openTrades.length > 0 ? (
        <ul className="mt-5 space-y-3 border-t border-white/10 pt-4">
          {openTrades.map((trade) => (
            <li key={trade.id} className="rounded-xl border border-white/10 bg-[#2D0A6B]/15 p-3">
              <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
                <span className="font-mono font-semibold text-[#00C9D4]">{trade.symbol}</span>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                <input
                  ...
                  className="min-w-[10rem] flex-1 rounded-lg border border-white/10 bg-[#2D0A6B]/20 px-3 py-2 text-sm text-white focus:border-[#00C9D4]/40 focus:outline-none"
                />
                <button
                  type="button"
