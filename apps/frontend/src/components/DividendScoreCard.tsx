...
const toneClasses: Record<GaugeTone, { bar: string; text: string }> = {
  red: { bar: "bg-terminal-negative", text: "text-terminal-negative" },
  yellow: { bar: "bg-amber-400", text: "text-amber-300" },
  green: { bar: "bg-terminal-positive", text: "text-terminal-positive" },
};
...
export function DividendScoreCard({ safetyScore, safetyReason }: DividendScoreCardProps) {
  ...
  return (
    <section className={TERMINAL_DIVIDEND_METRIC}>
      <h2 className={TERMINAL_SECTION_TITLE}>Safety score</h2>
      <div className="mt-4 flex items-end gap-3">
        <span className={`text-5xl font-bold tabular-nums ${text}`}>{clamped}</span>
        <span className="pb-1 text-sm text-terminal-textMuted">/ 100</span>
      </div>
      <div className="mt-4 h-3 w-full overflow-hidden rounded-full bg-terminal-panelSecondary">
        <div className={`h-full rounded-full transition-all ${bar}`} style={{ width: `${clamped}%` }} />
      </div>
      <p className={`mt-4 ${TERMINAL_TEXT_MUTED} leading-relaxed`}>{safetyReason}</p>
    </section>
  );
}
